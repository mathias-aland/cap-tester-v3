...
// IMPORTANT! Sized to hold one OUTPUT report.
SI_SEGMENT_VARIABLE(usb_rxBuf[USB_HID_OUT_SIZE], uint8_t, SI_SEG_XDATA);

// Linear buffer holds INPUT report to transmit.
// IMPORTANT! Sized to hold one INPUT report.
SI_SEGMENT_VARIABLE(usb_txBuf[USB_HID_IN_SIZE], uint8_t, SI_SEG_XDATA);
...
void USB_initModule(void)
{
  ...
  // Restore the SFR page before leaving
  SFRPAGE = SFR_save;
...
static void readReportData(uint8_t addr, uint8_t count)
{
  ...
  // Copy selected FIFO to circular receive buffer.

  while (count)
  {
	  if (usb_rxHead == 64)
	  {
		  // buffer full, throw away remaining data
		  USB_readDataReg();
	  }
	  else
	  {
